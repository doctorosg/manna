...
class BotService {
    private let botNames = [
        "FaithWalker","PsalmSinger","ScrollSeeker","GraceNote","TruthFinder",
        "CrossRoad","SpiritWind","LightBearer","HolyQuest","BibleBuff",
        "VerseKing","ProphetPro","SaltLight","DawnStar","AmenMind","PathFinder",
    ...
    func generateOpponents(count: Int, mode: GameMode) -> [BotOpponent] {
        ...
        for _ in 0..<count {
            var name: String
            repeat { name = botNames.randomElement() ?? "Player" } while used.contains(name)
            used.insert(name)
            ...
            opponents.append(BotOpponent(name: name, countryFlag: flags.randomElement() ?? "🏳️", scorePercentage: skill, league: league))
        }
        return opponents
    }

    func simulateAnswers(for question: MannaQuestion, bots: [BotOpponent]) -> [BotAnswer] {
        bots.map { bot in
            let penalty = Double(question.difficultyLevel.rawValue - 1) * 8.0
            ...
            return BotAnswer(botName: bot.name, botScore: bot.scorePercentage, selectedIndex: idx, isCorrect: correct, responseTime: time)
        }
    }
}
